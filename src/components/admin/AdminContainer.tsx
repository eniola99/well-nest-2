import React from "react";
import { JobOpeningContent } from "./JobOpenings";
import { DashboardContent } from "./Dashboard";
import { IJobTemplate } from "@/src/utils/utils";

interface IDashboardIndex {
  [key: string]: string;
}

interface IAdminContainer {
  activeTab: IDashboardIndex | undefined;
  isFetching?: boolean;
  error?: string | null;
  jobList: IJobTemplate[] | undefined;
}
export const AdminContainer = ({
  activeTab,
  jobList,
  isFetching,
  error,
}: IAdminContainer) => {
  let PageComponent: React.ElementType;

  switch (true) {
    case activeTab && activeTab.name === "Dashboard":
      PageComponent = DashboardContent;
      break;
    case activeTab && activeTab.name === "Job Openings":
      PageComponent = JobOpeningContent;
      break;
    default:
      PageComponent = DashboardContent;
      break;
  }
  return (
    <>
      <main className="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 className="h2">Dashboard</h1>
          <div className="btn-toolbar mb-2 mb-md-0">
            <div className="btn-group me-2">
              {/* <button
                type="button"
                className="btn btn-sm btn-outline-secondary"
              >
                Share
              </button>
              <button
                type="button"
                className="btn btn-sm btn-outline-secondary"
              >
                Export
              </button> */}
            </div>
            {/* <button
              type="button"
              className="btn btn-sm btn-outline-secondary dropdown-toggle"
            >
              <span data-feather="calendar"></span>
              This week
            </button> */}
          </div>
        </div>
        <PageComponent
          jobList={jobList}
          activeTab={activeTab}
          isFetching={isFetching}
          error={error}
        />
      </main>
    </>
  );
};
